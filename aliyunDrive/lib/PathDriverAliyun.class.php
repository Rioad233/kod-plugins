...
class PathDriverAliyun extends PathDriverBase {
    ...
    public function mkdir($dir,$repeat=REPEAT_SKIP) {
        $this->logOut("mkdir:".json_encode(array(
                "dir"=>$dir,
                "repeat"=>$repeat,
            )));
    ...
    private function fileManager($fileList, $action){
        $this->logOut("fileManager:".json_encode(array(
                "fileList"=>$fileList,
                "action"=>$action,
            )));
        if($action == "rename"){
    ...
    public function copyFile($from,$to) {
        $this->logOut("copyFile:".json_encode(array(
                "from"=>$from,
                "to"=>$to,
            )));
    ...
    public function moveFile($from, $to) {
        $this->logOut("moveFile:".json_encode(array(
                "from"=>$from,
                "to"=>$to,
            )));
        $dest = get_path_father($to);
    ...
    public function delFile($path) {
        $this->logOut("delFile:".json_encode(array(
                "$path"=>$path
            )));
    ...
    public function delFolder($path) {
        $this->logOut("delFolder:".json_encode(array(
                "$path"=>$path
            )));
    ...
    public function rename($from, $to) {
        $this->logOut("rename:".json_encode(array(
                "from"=>$from,
                "to"=>$to
            )));
        $fileList = array(
    ...
    private function getDownUrl($file_id){
        $this->logOut("getDownUrl:".json_encode(array(
                "file_id"=>$file_id
            )));
        $param = array("file_id"=>$file_id,"drive_id"=>$this->defaultDriveId);
        $headers = array("Content-Type: application/json",
    ...
    private function getRefererUrl($url,$fileName){
        ...
        if($this->strEndWith($tmpUrl,"index.php")){
    ...
    public function fileInfo($file,$simple=false,$fileInfo = array(),$path) {
        ...
        $info = array(
            ...
            'isReadable'	 => true,
            'isWriteable'	 => true,
            'fileThumb' => $fileInfo['thumbnail'] ? $this->getRefererUrl($fileInfo['thumbnail'],$fileName):null
        );
        $this->logOut("fileInfo:".json_encode(array(
                "file"=>$file,
                "simple"=>$simple,
                "fileInfo"=>$fileInfo,
                "path"=>$path,
    ...
    public function folderInfo($path,$simple=false,$itemInfo=array(),$path2) {
        ...
        $this->logOut("folderInfo:".json_encode(array(
                "path"=>$path,
                "simple"=>$simple,
                "itemInfo"=>$itemInfo,
                "path2"=>$path2,
    ...
    private function path2id($path){
        $this->logOut("path2id:".json_encode(array(
                "path"=>$path
            )));
        if($path == "" || $path == "/"){
            return "root";
    ...
    public function listPath($path,$simple=false) {
        $this->logOut("listPath:".json_encode(array(
                "path"=>$path,
                "simple"=>$simple
            )));
        $start = 0;// 起始位置
    ...
    public function info($path){
        if($this->isFolder($path)){
            return $this->folderInfo($path,null,null,null);
        }else if($this->isFile($path)) {
            return $this->fileInfo($path,null,null,null);
        }
        return false;
    }
